...
class OrquestadorDesarrollo:
    ...
    def verificar_conectividad_servicio(self, servicio: str, puerto: int, timeout: int = 10) -> bool:
        ...
        try:
            ...
            elif servicio == 'pgadmin':
                # Usar endpoint de ping para PgAdmin
                url = f"http://localhost:{puerto}/misc/ping"
            else:
                url = f"http://localhost:{puerto}"
    ...
    def diagnosticar(self) -> Dict[str, Dict]:
        ...
        for servicio, config in self.servicios_esperados.items():
            ...
            servicio_info = {
                ...
                'conectividad': False,
                'opcional': config.get('opcional', False),
        ...
        servicios_saludables = sum(1 for s in servicios_requeridos.values() 
                                 if s['estado'] in ['healthy', 'running'] and s['conectividad'])
        servicios_ejecutando = sum(1 for s in servicios_requeridos.values() 
                                 if s['estado'] in ['healthy', 'running'])
        total_servicios = len(servicios_requeridos)
        ...
        # Contar servicios opcionales
        servicios_opcionales_ejecutando = sum(1 for s in diagnostico['servicios'].values() 
                                            if s.get('opcional', False) and s['estado'] in ['healthy', 'running'])
        total_servicios_opcionales = sum(1 for s in diagnostico['servicios'].values() 
                                       if s.get('opcional', False))
        
        diagnostico['resumen'] = {
            'servicios_saludables': servicios_saludables,
            'servicios_ejecutando': servicios_ejecutando,
            'total_servicios': total_servicios,
            'servicios_opcionales_ejecutando': servicios_opcionales_ejecutando,
            'total_servicios_opcionales': total_servicios_opcionales,
            'total_servicios_incluye_opcionales': total_servicios_incluye_opcionales,
