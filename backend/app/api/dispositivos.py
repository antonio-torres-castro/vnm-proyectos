...
from app.schemas.dispositivos import (
    DispositivosResponse, DispositivosDetallado, DispositivosListResponse,
    DispositivosFiltros, DispositivosEstadisticas
)
...
async def get_dispositivos(
    ...
    devstatus: Optional[int] = Query(None, description="Filtrar por estado del dispositivo"),
    solo_activos: bool = Query(False, description="Solo dispositivos activos (UP)"),
    con_geolocalizacion: bool = Query(False, description="Solo dispositivos con geolocalización"),
...
async def buscar_dispositivos(
    ...
    # Convertir a response detallado
    dispositivos_detallados = [DispositivosDetallado(**dispositivo.__dict__) for dispositivo in dispositivos]
...
async def get_dispositivo(
    ...
    dispositivos_info, _ = DispositivosService.get_with_interfaces_count(
        ...
        skip=0,
        limit=1,
        filtros=DispositivosFiltros(devstatus=None)  # Sin filtros para obtener el específico
    )
...
async def get_dispositivos_mapa(
    ...
    dispositivos, total = DispositivosService.get_all(
        db=db,
        skip=0,
        limit=10000,  # Límite alto para mapa
        filtros=filtros
    )
    ...
    return {
        "total": total,
        "dispositivos": dispositivos_mapa
    }
