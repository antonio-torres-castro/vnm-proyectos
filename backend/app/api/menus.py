...
async def get_menu_grupo(
    ...
    if not grupo:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Grupo de menú no encontrado"
        )
    return grupo


@router.post(
    "/grupos", response_model=MenuGrupoResponse, status_code=status.HTTP_201_CREATED
)
...
async def get_menu(
    ...
    if not menu:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Menú no encontrado"
        )
    return menu
...
async def reorder_menus(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Error reordenando menús"
        )
...
async def reorder_grupos(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Error reordenando grupos"
        )
