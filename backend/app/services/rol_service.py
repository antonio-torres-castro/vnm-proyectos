...
class RolService:
    ...
    def get_menus(db: Session, rol_id: int) -> List[Menu]:
        ...
        return db.query(Menu).join(RolMenu).filter(
            and_(
                RolMenu.rol_id == rol_id,
                RolMenu.estado_id == 1  # Activo
            )
    ...
    def assign_menu(db: Session, asignacion: RolMenuCreate) -> bool:
        """Asignar menú a rol"""
        # Verificar si ya existe la asignación
        existing = db.query(RolMenu).filter(
            and_(
    ...
    def remove_menu(db: Session, rol_id: int, menu_id: int) -> bool:
        """Remover menú de rol"""
        asignacion = db.query(RolMenu).filter(
            and_(
                RolMenu.rol_id == rol_id,
                RolMenu.menu_id == menu_id
            )
        ).first()
    ...
    def has_permission(db: Session, rol_id: int, permiso_nombre: str) -> bool:
        ...
        return db.query(RolPermiso).join(Permiso).filter(
            and_(
                RolPermiso.rol_id == rol_id,
    ...
    def has_menu_access(db: Session, rol_id: int, menu_url: str) -> bool:
        ...
        return db.query(RolMenu).join(Menu).filter(
            and_(
                RolMenu.rol_id == rol_id,
                RolMenu.estado_id == 1,
                Menu.url == menu_url,
