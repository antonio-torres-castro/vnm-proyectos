...
from app.schemas.menu import (
    MenuCreate, MenuUpdate, MenuGrupoCreate, MenuGrupoUpdate
)
...
class MenuService:
    ...
    @staticmethod
    def update_grupo(db: Session, grupo_id: int, grupo_data: MenuGrupoUpdate) -> Optional[MenuGrupo]:
    ...
    def get_menu_by_id(db: Session, menu_id: int) -> Optional[Menu]:
        ...
        return db.query(Menu).options(
            joinedload(Menu.grupo)
        ).filter(Menu.id == menu_id).first()
    
    @staticmethod
    def get_all_menus(db: Session, grupo_id: Optional[int] = None, activos_solo: bool = True) -> List[Menu]:
    ...
    def get_menus_by_role(db: Session, rol_id: int) -> List[MenuGrupo]:
        ...
        grupos = db.query(MenuGrupo).filter(MenuGrupo.estado_id == 1).order_by(MenuGrupo.orden).all()
        
        # Para cada grupo, obtener menús accesibles
        for grupo in grupos:
            menus_accesibles = db.query(Menu).join(RolMenu).filter(
                and_(
        ...
        grupos_con_menus = [grupo for grupo in grupos if hasattr(grupo, 'menus_accesibles') and grupo.menus_accesibles]
