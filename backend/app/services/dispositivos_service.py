...
class DispositivosService:
    ...
    def get_all(
        ...
        if filtros:
            if filtros.operador:
                query = query.filter(Dispositivos.operador.ilike(f"%{filtros.operador}%"))
            ...
            if filtros.enterprise:
                query = query.filter(Dispositivos.enterprise.ilike(f"%{filtros.enterprise}%"))
        ...
        dispositivos = query.order_by(asc(Dispositivos.devname)).offset(skip).limit(limit).all()
    ...
    def get_estadisticas(db: Session) -> Dict[str, Any]:
        ...
        stats_estado = db.query(
            Dispositivos.devstatus,
            func.count(Dispositivos.devid).label('count')
        ).group_by(Dispositivos.devstatus).all()
        
        # Estadísticas por operador
        stats_operador = db.query(
            Dispositivos.operador,
            func.count(Dispositivos.devid).label('count')
        ).filter(Dispositivos.operador.isnot(None)).group_by(Dispositivos.operador).all()
        
        # Estadísticas por zona
        stats_zona = db.query(
            Dispositivos.zona,
            func.count(Dispositivos.devid).label('count')
        ).filter(Dispositivos.zona.isnot(None)).group_by(Dispositivos.zona).all()
        
        # Estadísticas por fabricante
        stats_enterprise = db.query(
            Dispositivos.enterprise,
            func.count(Dispositivos.devid).label('count')
        ).filter(Dispositivos.enterprise.isnot(None)).group_by(Dispositivos.enterprise).all()
        
        # Calcular totales
        total_dispositivos = db.query(func.count(Dispositivos.devid)).scalar()
        dispositivos_up = db.query(func.count(Dispositivos.devid)).filter(Dispositivos.devstatus == 1).scalar()
        dispositivos_down = db.query(func.count(Dispositivos.devid)).filter(Dispositivos.devstatus == 2).scalar()
        dispositivos_no_responden = db.query(func.count(Dispositivos.devid)).filter(Dispositivos.devstatus == 0).scalar()
        dispositivos_fuera_monitoreo = db.query(func.count(Dispositivos.devid)).filter(Dispositivos.devstatus == 5).scalar()
        
        # Calcular porcentaje de disponibilidad
        porcentaje_disponibilidad = (dispositivos_up / total_dispositivos * 100) if total_dispositivos > 0 else 0
    ...
    def get_with_interfaces_count(
        ...
        query = db.query(
            ...
            ).label('interfaces_activas')
        ).outerjoin(Interfaces).group_by(Dispositivos.devid)
        ...
        # Aplicar paginación
        resultados = query.order_by(asc(Dispositivos.devname)).offset(skip).limit(limit).all()
    ...
    def get_valores_filtros(db: Session) -> Dict[str, List[str]]:
        ...
        operadores = db.query(Dispositivos.operador).filter(
            Dispositivos.operador.isnot(None)
        ).distinct().order_by(Dispositivos.operador).all()
        
        zonas = db.query(Dispositivos.zona).filter(
            Dispositivos.zona.isnot(None)
        ).distinct().order_by(Dispositivos.zona).all()
        
        hubs = db.query(Dispositivos.hub).filter(
            Dispositivos.hub.isnot(None)
        ).distinct().order_by(Dispositivos.hub).all()
        
        areas = db.query(Dispositivos.area).filter(
            Dispositivos.area.isnot(None)
        ).distinct().order_by(Dispositivos.area).all()
        
        enterprises = db.query(Dispositivos.enterprise).filter(
            Dispositivos.enterprise.isnot(None)
        ).distinct().order_by(Dispositivos.enterprise).all()
