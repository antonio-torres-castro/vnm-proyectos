...
def check_primitive_tables_empty(db: Session) -> bool:
    ...
    Las tablas primitivas (estados, permisos, rol) se poblan únicamente
    por scripts SQL.
    """
    try:
...
def init_estados(db: Session):
    ...
    estados_basicos = [
        ...
        {"id": 2, "nombre": "Activo", "descripcion": "Registro activo en el sistema"},
        {
            "id": 3,
            "nombre": "Inactivo",
            "descripcion": "Registro inactivo en el sistema",
        },
        {
            "id": 4,
            "nombre": "Eliminado",
            "descripcion": "Registro eliminado logicamente en el sistema",
        },
        {
            "id": 5,
            "nombre": "Iniciado",
...
def init_roles(db: Session):
    ...
    if db.query(Rol).count() > 0:
        logger.warning(
            "Tabla rol ya contiene registros - no se puede poblar desde código"
        )
        return
    ...
    roles_basicos = [
        ...
        },
        {
            "id": 3,
            "nombre": "Ejecutor",
            "descripcion": "Ejecutor básico con permisos de solo lectura",
        },
    ]
